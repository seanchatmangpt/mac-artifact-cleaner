...
fn deletion_and_thin_projections_are_admitted_by_fleet_validator() {
    ...
    let ctx = context_for(&native, "test-approver", "plan-approval hmac:test", 0).unwrap();
    ...
    let ctx = context_for(&native, "oclnr", "pressure-policy: test", 0).unwrap();
...
fn refused_projection_is_still_schema_valid() {
    ...
    let ctx = context_for(&native, "oclnr", "", 0).unwrap();
