...
//! serialization. The inert [`OclArtifact`] record type lives in
//! `crate::domain::ocl` (primitives only, no `sled` types) — matching the
//! split used for the scan cache in `crate::integration::scan_cache`.
...
use crate::domain::ocl::OclArtifact;
...
impl OclDatabase {
    ...
    ///
    /// ```
    /// use osx_clnr::integration::ocl_store::OclDatabase;
    ///
