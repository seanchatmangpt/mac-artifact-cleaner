...
\section{Formalizing Filesystem Lifecycle
Semantics:}\label{formalizing-filesystem-lifecycle-semantics}

\subsection{An Object-Centric Process Mining Framework for Autonomic
Artifact
Management}\label{an-object-centric-process-mining-framework-for-autonomic-artifact-management}

\textbf{A Dissertation Submitted for the Degree of Doctor of Philosophy
in Computer Science} \textbf{Candidate:} Sean Chatman \textbf{Context:}
Vision 2030 Process Intelligence Architecture
...
alignment-based conformance checking over \(N \geq 1,000\) traces. We
...
\section{Glossary of Foundational Axioms, Laws, and
...
\subsubsection{1. Rice's Theorem (Static Analysis
...
\subsubsection{2. The Curry-Howard Isomorphism (Propositions as
...
\subsubsection{\texorpdfstring{3. Little's Law
...
\subsubsection{4. Cryptographic Collision Resistance (The Birthday
...
\subsubsection{5. The Bellman Equation (Dynamic
...
\subsubsection{6. Soundness of Workflow Nets (Petri Net
...
\subsection{1.1 The Entropic Nature of Modern
...
\subsection{1.2 The Order Parameter is
...
\subsection{1.3 The Three Phase-Change
...
\subsection{1.4 Research Questions}\label{research-questions}
...
\subsection{1.5 The Mundanity of the Proof
...
\section{Chapter 2: Literature Review and State of the
Art}\label{chapter-2-literature-review-and-state-of-the-art}

\subsection{2.1 The Evolution of Process
...
\subsection{2.2 Object-Centric Process Mining (OCPM) and OCEL
...
\subsection{2.3 Typestate Programming and Compile-Time
...
\subsection{2.4 Synthesis: Typestate-Enforced Process
...
\subsection{2.5 The Universal Chatman Equation and Cross-Domain
...
\section{Chapter 3: Mathematical Formalisms and
Ontologies}\label{chapter-3-mathematical-formalisms-and-ontologies}

\subsection{3.1 Formalizing the Filesystem as an OCEL 2.0
...
\subsubsection{3.1.1 Completeness and Soundness of the OCEL
...
\textbf{Lemma 1 (Completeness):} Every POSIX operation that materially
changes a tracked artifact maps to some \(e \in E\). By hooking into the
filesystem polling layer (and strictly gating execution via the
\texttt{Admit} boundary), no destructive or exclusionary OS event within
the governed domains drops silently. * \textbf{Lemma 2 (Soundness):}
Every \(e \in E\) corresponds to an actual POSIX operation that
occurred. Because \(E\) emissions are inextricably bound to
cryptographic receipt creation, no phantom events can be generated. The
emission of \(e\) requires the resolution of its physical OS
counterpart.

\subsection{3.2 Formal OCPN
Construction}\label{formal-ocpn-construction}
...
\textbf{variable arc weights}, allowing a single transition to
dynamically consume and produce an arbitrary number of object tokens
representing diverse filesystem artifacts. * \textbf{Marking \(M_0\):}
The initial state representing identified artifacts on disk.
...
\subsection{3.3 The Gall Checkpoint Pipeline and LTL
...
\section{Chapter 4: System Architecture and
Implementation}\label{chapter-4-system-architecture-and-implementation}

\subsection{4.1 The Main Theorems: Typestate-OCEL Safety and its
...
\subsection{4.2 Deletion as a Receipt-Bearing Process
...
\subsection{4.3 Cryptographic Execution Receipts (Unforgeable
...
\subsection{4.4 Complexity Analysis}\label{complexity-analysis}
...
\subsection{5.1 Baseline Comparison}\label{baseline-comparison}
...
\textbf{Causal Discovery} & No & No & No & Yes (Inductive OCPN) \\
...
\subsection{5.2 Conformance Checking
...
\subsection{5.4 Cache Thrashing Quantification (Lean
...
observed a Cache Thrashing rate of \textbf{18.4\%}. Developers were
actively wasting disk I/O and network bandwidth redownloading identical
\texttt{target/} objects. After transitioning governance to the
\texttt{osx-clnr} \texttt{DeletionPlanAdjudicator}---which enforces
semantic tool root awareness---the Cache Thrashing rate plummeted to
\textbf{0.6\%}. This validates RQ4, demonstrating massive efficiency
gains when shifting from static measurement to process intelligence.
define ``Cache Thrashing'' as the anti-pattern where an artifact is
deleted to free space, only to be immediately re-acquired due to broken
implicit build dependencies. Let \(x\) be an artifact. Thrashing is
defined by the temporal logic expression:
\[\text{Thrashing}(x) \equiv \text{deletion}(x) \wedge \lozenge_{\leq T} \text{re-acquisition}(x)\]
where \(T = 72 \text{ hours}\).
...
\section{Chapter 6: Conclusion and Future
...
\subsection{6.2 Future Work: Formal Autonomic RL
